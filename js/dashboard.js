...
function handleRouteChange() {
    ...
    if (path === '/source-selector') {
        ...
        if (catId) {
            currentCatId = parseInt(catId, 10);
...
function renderCategoryList() {
    ...
    db.categories.forEach((cat, index) => {
        ...
        div.onclick = () => {
            if (!isCatSortMode) {
                navigateTo('/problems?catId=' + cat.id);
            }
        };
...
function renderProblemList() {
    ...
    filtered.forEach((p) => {
        ...
        div.onclick = () => {
            if (!isBankSortMode) {
                window.location.href = '/workspace?probId=' + p.id;
            }
        };
        list.appendChild(div);
    });
...
function openAdmin(probId) {
    window.location.href = '/admin?probId=' + probId;
}
...
function editProblemInList(e, id) { 
        ...
        window.location.href = '/admin?probId=' + id; 
    }
